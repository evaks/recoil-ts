...
import {Corner, Overflow} from "./positioning.ts";
import {AnchoredViewportPosition} from "./anchoredviewportposition.ts";
...
export class MenuAnchoredPosition extends AnchoredViewportPosition {
    ...
    constructor(anchorElement: Element, corner: Corner, opt_adjust?: boolean, opt_resize?: boolean) {
        ...
        if (opt_adjust || opt_resize) {
            let overflowX = Overflow.ADJUST_X_EXCEPT_OFFSCREEN;
            let overflowY = !opt_resize ? Overflow.ADJUST_Y_EXCEPT_OFFSCREEN : Overflow.RESIZE_HEIGHT;
