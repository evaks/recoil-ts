...
import {EventType, Transition} from "./transition";
 ...
 */
export abstract class TransitionBase extends EventTarget implements Transition{
  /**
   * The internal state of the animation.
   */
  private state_ = State.STOPPED;
  /**
   * Timestamp for when the animation was started.
   */
  protected startTime: null|number = null;
   ...
   */
  protected setStateStopped() {
    this.state_ = State.STOPPED;
  };


  /**
   * @return True iff the current state of the animation is playing.
   */
  isPlaying():boolean {
  ...
  /**
   * @return True iff the current state of the animation is paused.
   */
  isPaused():boolean {
  ...
  /**
   * @return True iff the current state of the animation is stopped.
   */
  isStopped():boolean {
   ...
   */
  protected dispatchAnimationEvent(type:string) {
    this.dispatchEvent(new Event(type));
  }

  abstract play(opt_restart?: boolean): boolean;

  abstract stop(opt_gotoEnd?: boolean): boolean;
